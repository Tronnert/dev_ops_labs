...
Итак, для поставленную задачу я решил делать на виртуалках c arch (так как я вероятно мазахист), так что через `archinstall` поднимаю мимнимальный arch_server и десктопный arch_client и прокидываю nat network между ними. У arch_server ip 10.0.2.16, у arch_client 10.0.2.22.
...
5. В теории dns сервер рабоает, но его пока никто не видит, для этого нужно настроить `/etc/resolv.conf` и, хоть это можно делать и руками, лучше воспользоваться `resolvconf`. Для этого установим его `sudo pacman -S openresolv` в /etc кинем `resolvconf.conf` - 
    ```
    resolv_conf=/etc/resolv.conf
    name_servers=10.0.2.16
    ```
    и запустить `sudo resolvconf -u`. Этот же шаг нужно повторить на клиенте.

Теперь можно посмотреть на эти доменные имена через nslookup и все должо работать:
...
Запустим два приложения на flask - они находятся в `/lab1/memes` и `lab1/songs`. Команда - `flask --app main run --host=0.0.0.0 --port=5000`. Для вторго приложения порт будет 5001.
...
Если бы Ленин был программистом он бы завещал: $${\color{red}Гуглите, гуглите\ и\ гуглите}$$

Потому что пришлось гуглить МНОГО. 

Почему `/lab1/memes` и `/lab1/songs` так называются? Потому что я хотел не просто зашлушки сделать, а архив с мемами и сайт для караоке, но это заняло бы слишком много времени (может когда нибудь).
