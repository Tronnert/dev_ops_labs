...
**Почему способ хороший?**
 ...
 - Секреты не хранятся напрямую, а хранятся на внешнем сервисе, который сам по себе следит за безопасностью хранимых данных

 - Секреты легко обновлять, так как в репозитории хранится только токен и адрес к Vault. Изменение секретов в самом Vault сразу позволяет использовать обновленные данные в любом месте, будь то пайплайн или локальные тесты


**Почему способ на самом деле не очень хороший?**

- Согласно документации по использоваию секретов, ```If a secret was used in the job, GitHub automatically redacts secrets printed to the log. You should avoid printing secrets to the log intentionally.```. То есть секреты выводятся в логах в зашифрованном виде, а это значит, что потенциально может возникнуть такая ситуация, что злоумышленник сможет, например, с помощью инъекции, получить секреты из логов.

- В секретах мы теперь храним самое главное - токен от доступа ко *всем* секретам. И если кто-то получит доступ к токену, то хана всему, что мы пытаемся спрятать. А получиьт доступ к токену можно как через логи, так и получив доступ к самому репозиторию (украв личные данные от входа в аккаунт на гитхабе)

- ngrok тоже достаточно небезопасный, а мы используем один туннель на протяжении всего "жизненного цикла" проекта.

**Итог:** Способ использования секретов в CI/CD-пайплайне с помошью секретохранилки, а не напрямую в Guthub secrets, рабочий и в целом может быть более безопасным, если над ним еще поработать (сделать настройки доступа в Vault хотя бы). Но есть и свои подводные камни. Самым безопасным способом будет хранение секретов на листе бумаги, но так как это неприменимо в промышленных масштабах, приходится идти на риски.
